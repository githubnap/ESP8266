...
String id_now = "";             // ID ที่ได้มาจากการแสกน RFID
String id_now_name = "";
  ...
  switch (page) {
    case 0:
      ...
      if (ReadInput() == "0001") {  // กดปุ่ม 1 เพื่อดูยอดขายวัน
        ...
        lcd.print("              ");
        lcd.setCursor(0, 1);
        lcd.print(String(p_tm -> tm_mon) + "Day:" + String(p_tm -> tm_mday) + "= " );
      ...
      if (ReadInput() == "0010") {  // กดปุ่ม 2 เพื่อดูยอดวันก่อน
        push_Header();
        time_t now = time(nullptr);
        struct tm* p_tm = localtime(&now);
        lcd.setCursor(0, 1);
        lcd.print("              ");
        lcd.setCursor(0, 1);
        int i = (p_tm -> tm_mday) - 1 ;
        
        if (i <= 0) {
          if ((p_tm -> tm_mon) == 4 && (p_tm -> tm_mon) == 6 && (p_tm -> tm_mon) == 9 && (p_tm -> tm_mon) == 11) {
            i = 30;
          } else
            i = 31;
          lcd.print(String(p_tm -> tm_mon) + "Day:" + String(i) + "= " );
          lcd.print(String(Firebase.getInt("Year:" + String((p_tm -> tm_year) - 100 + 2000) + "/Month" + String(p_tm -> tm_mon) + "/Balance/Day:" + i + "/Sell_Day")));

        } else {
          lcd.print(String(p_tm -> tm_mon) + "Day:" + String(i) + "= " );
          lcd.print(String(Firebase.getInt(Header + "Balance/Day:" + String(i) + "/Sell_Day")));
        }
        delay(1000);
        lcd.setCursor(0, 1);
        lcd.print("              ");
      ...
      if (ReadInput() == "0011") {  // กดปุ่ม 3 เพื่อดูยอดขายเดือน
        push_Header();
        time_t now = time(nullptr);
        struct tm* p_tm = localtime(&now);
        lcd.setCursor(0, 1);
    ...
    case 1:
      ...
      lcd.print(id_now);
      lcd.setCursor(0, 0);
      Firebase.setString("DataBase/Staff_Name/New_ID", id_now);      //แสกนบัตรใหม่แล้วเพิ่มเข้าไป
      lcd.print("ID:");
      id_now_name = Firebase.getString("DataBase/Staff_Name/" + id_now);
